...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 07-11-2025 a las 09:10:56
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_enviar_notificaciones_inmediatas` ()   BEGIN
                DECLARE done INT DEFAULT FALSE;
                DECLARE notif_id INT;
                DECLARE destinatario_email VARCHAR(255);
                DECLARE asunto TEXT;
                DECLARE mensaje TEXT;
                
                DECLARE cur CURSOR FOR 
                    SELECT id, destinatario_email, asunto, mensaje 
                ...
                read_loop: LOOP
                    FETCH cur INTO notif_id, destinatario_email, asunto, mensaje;
                    
                    IF done THEN
                        LEAVE read_loop;
                    END IF;
                    
                    -- Marcar como enviado (esto simula el envío)
                    -- En un sistema real, aquí iría la lógica de envío
...
(47, NULL, 2, '2025-11-07 04:35:05', 8, 12, 0, 'registrado', NULL, 0),
(48, NULL, 1, '2025-11-07 07:49:30', 11, 7, 0, 'resuelto', NULL, 1),
(49, NULL, 1, '2025-11-07 07:54:05', 11, 7, 0, 'registrado', NULL, 0);

--
-- Disparadores `faltantes`
--
DELIMITER $$
CREATE TRIGGER `trg_notificacion_faltante_inmediata` AFTER INSERT ON `faltantes` FOR EACH ROW BEGIN
        DECLARE config_activo_val INT DEFAULT 0;
        DECLARE config_frecuencia_val VARCHAR(20) DEFAULT '';
        
        -- Obtener configuración
        SELECT config_activo, config_frecuencia 
...
(1, 1, 'email', 1, '2025-11-06 18:28:41', '2025-11-07 07:53:35', 'inmediata', NULL, '[\"1\",\"2\",\"3\",\"4\",\"5\"]', NULL),
...
(17, 'email', 'fabricio.gomez4371@gmail.com', NULL, NULL, '🚨 ALERTA: Nuevo faltante detectado', 'Se ha detectado un nuevo faltante en el inventario.\n\nDetalles:\n• ID: 47\n• Cantidad faltante: 12 unidades\n• Estado: detectado\n• Fecha: 2025-11-07 01:35:05\n\nIngrese al sistema para revisar los detalles y generar pedidos.\n\nSistema AXSFITT', NULL, '2025-11-07 04:35:05', '2025-11-07 04:36:24', 'enviado', NULL),
(18, 'email', 'fabricio.gomez4371@gmail.com', 'fabricio gomez', NULL, 'ALERTA: Stock bajo - Whey Protein  - Sabor: Frutilla', 'Se ha detectado stock bajo en el siguiente producto:\n\nProducto: Whey Protein  - Sabor: Frutilla\nStock actual: 8\nStock mínimo: 9\nCantidad faltante: 12\n\nSe recomienda realizar un pedido lo antes posible.\n\nSistema de Gestión AXSFITT', 47, '2025-11-07 04:35:09', '2025-11-07 04:36:36', 'enviado', NULL),
(19, 'email', 'pedro.martinez@empresa.com', 'Pedro Martinez', NULL, 'ALERTA: Stock bajo - Whey Protein  - Sabor: Frutilla', 'Se ha detectado stock bajo en el siguiente producto:\n\nProducto: Whey Protein  - Sabor: Frutilla\nStock actual: 8\nStock mínimo: 9\nCantidad faltante: 12\n\nSe recomienda realizar un pedido lo antes posible.\n\nSistema de Gestión AXSFITT', 47, '2025-11-07 04:35:09', '2025-11-07 04:36:39', 'enviado', NULL),
(20, 'email', 'fabricio.gomez4371@gmail.com', NULL, NULL, '🚨 ALERTA: Nuevo faltante detectado', 'Se ha detectado un nuevo faltante en el inventario.\n\nDetalles:\n• ID: 49\n• Cantidad faltante: 7 unidades\n• Estado: detectado\n• Fecha: 2025-11-07 04:54:05\n\nIngrese al sistema para revisar los detalles y generar pedidos.\n\nSistema AXSFITT', NULL, '2025-11-07 07:54:05', NULL, 'pendiente', NULL),
(21, 'email', 'fabricio.gomez4371@gmail.com', 'fabricio gomez', NULL, 'ALERTA: Stock bajo - Whey Protein  - Sabor: Vainilla', 'Se ha detectado stock bajo en el siguiente producto:\n\nProducto: Whey Protein  - Sabor: Vainilla\nStock actual: 11\nStock mínimo: 12\nCantidad faltante: 7\n\nSe recomienda realizar un pedido lo antes posible.\n\nSistema de Gestión AXSFITT', 49, '2025-11-07 07:54:30', NULL, 'pendiente', NULL),
(22, 'email', 'pedro.martinez@empresa.com', 'Pedro Martinez', NULL, 'ALERTA: Stock bajo - Whey Protein  - Sabor: Vainilla', 'Se ha detectado stock bajo en el siguiente producto:\n\nProducto: Whey Protein  - Sabor: Vainilla\nStock actual: 11\nStock mínimo: 12\nCantidad faltante: 7\n\nSe recomienda realizar un pedido lo antes posible.\n\nSistema de Gestión AXSFITT', 49, '2025-11-07 07:54:30', NULL, 'pendiente', NULL);
...
(1, NULL, 1, 11, NULL, '2025-11-07 07:54:05', 12, 18),
(2, NULL, 2, 8, NULL, '2025-11-07 04:35:05', 9, 20),
(3, 2, NULL, 17, NULL, '2025-11-07 02:59:56', 21, 50),
...
--
-- Estructura Stand-in para la vista `v_faltantes_notificacion`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_faltantes_notificacion` (
`faltante_id` int(10) unsigned
,`producto_nombre` mediumtext
,`producto_id` int(10) unsigned
,`faltante_variante_id` int(10) unsigned
,`faltante_cantidad_faltante` int(10) unsigned
,`stock_actual` int(10) unsigned
,`stock_minimo` int(10) unsigned
,`stock_maximo` int(10) unsigned
,`faltante_fecha_deteccion` timestamp
,`faltante_estado` enum('detectado','registrado','solicitado_parcial','solicitado_completo','pedido_generado','en_transito','resuelto')
,`faltante_resuelto` tinyint(1)
...
--
-- Estructura para la vista `v_faltantes_notificacion`
--
DROP TABLE IF EXISTS `v_faltantes_notificacion`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_faltantes_notificacion`  AS SELECT `f`.`faltante_id` AS `faltante_id`, CASE WHEN `f`.`faltante_producto_id` is not null THEN `p`.`producto_nombre` WHEN `f`.`faltante_variante_id` is not null THEN CASE WHEN group_concat(concat(`a`.`atributo_nombre`,': ',`vv`.`valor_nombre`) separator ', ') is not null THEN concat(`p2`.`producto_nombre`,' - ',group_concat(concat(`a`.`atributo_nombre`,': ',`vv`.`valor_nombre`) separator ', ')) ELSE concat(`p2`.`producto_nombre`,' (Variante ',`f`.`faltante_variante_id`,')') END END AS `producto_nombre`, CASE WHEN `f`.`faltante_producto_id` is not null THEN `f`.`faltante_producto_id` WHEN `f`.`faltante_variante_id` is not null THEN `v`.`producto_id` END AS `producto_id`, `f`.`faltante_variante_id` AS `faltante_variante_id`, `f`.`faltante_cantidad_faltante` AS `faltante_cantidad_faltante`, `s`.`cantidad` AS `stock_actual`, `s`.`stock_minimo` AS `stock_minimo`, `s`.`stock_maximo` AS `stock_maximo`, `f`.`faltante_fecha_deteccion` AS `faltante_fecha_deteccion`, `f`.`faltante_estado` AS `faltante_estado`, `f`.`faltante_resuelto` AS `faltante_resuelto`, CASE WHEN `f`.`faltante_producto_id` is not null THEN (select `ip`.`imagen_url` from `imagenes_productos` `ip` where `ip`.`producto_id` = `f`.`faltante_producto_id` and `ip`.`imagen_orden` = 0 limit 1) WHEN `f`.`faltante_variante_id` is not null THEN CASE WHEN `ip_var`.`imagen_url` is not null THEN `ip_var`.`imagen_url` ELSE (select `ip2`.`imagen_url` from `imagenes_productos` `ip2` where `ip2`.`producto_id` = `v`.`producto_id` and `ip2`.`imagen_orden` = 0 limit 1) END END AS `imagen_url` FROM (((((((`faltantes` `f` left join `productos` `p` on(`f`.`faltante_producto_id` = `p`.`producto_id`)) left join `variantes` `v` on(`f`.`faltante_variante_id` = `v`.`variante_id`)) left join `productos` `p2` on(`v`.`producto_id` = `p2`.`producto_id`)) left join `valores_variantes` `vv` on(`v`.`variante_id` = `vv`.`variante_id`)) left join `atributos` `a` on(`vv`.`atributo_id` = `a`.`atributo_id`)) left join `imagenes_productos` `ip_var` on(`v`.`imagen_id` = `ip_var`.`imagen_id`)) left join `stock` `s` on(`s`.`producto_id` = `f`.`faltante_producto_id` or `s`.`variante_id` = `f`.`faltante_variante_id`)) WHERE `f`.`faltante_estado` = 'detectado' AND `f`.`faltante_resuelto` = 0 GROUP BY `f`.`faltante_id`, `f`.`faltante_producto_id`, `f`.`faltante_variante_id` ORDER BY `f`.`faltante_fecha_deteccion` DESC ;
...
--
ALTER TABLE `faltantes`
  MODIFY `faltante_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=50;
